...
const Header = () => {
  ...
  return (
    <header className={cn('fixed top-0 left-0 right-0 z-50 transition-all duration-300 ease-in-out', isScrolled ? 'py-3 glass' : 'py-5 bg-transparent')}>
      <div className="container mx-auto px-4 md:px-8 flex items-center justify-between gap-4">
        <div className="flex items-center gap-4 lg:gap-6 min-w-0 flex-1">
          <a href="/" className="flex items-center gap-2 group shrink-0">
            <motion.div whileHover={{ rotate: 10 }} className="w-10 h-10 rounded-full bg-primary flex items-center justify-center text-white font-bold text-xl shadow-lg">M</motion.div>
            <span className="text-xl md:text-2xl font-bold tracking-tight text-foreground group-hover:text-primary transition-colors">Morning Fruit</span>
          </a>
          ...
          <div className="hidden lg:flex flex-1 max-w-md items-center rounded-full border border-border/60 bg-white/80 backdrop-blur-md px-4 py-2 shadow-sm">
            ...
            <input type="text" placeholder="Tìm kiếm trái cây, combo..." className="ml-3 w-full bg-transparent text-sm text-foreground placeholder:text-foreground/40 focus:outline-none" />
          </div>
        </div>
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((link) =>
            link.name === 'Trái cây' ? (
              <div key={link.name} className="relative" ref={fruitMenuRef} onMouseEnter={() => setFruitMenuOpen(true)} onMouseLeave={() => setFruitMenuOpen(false)}>
                <Link to={link.href} className="flex items-center gap-1 text-foreground/80 hover:text-primary font-medium transition-colors relative group text-sm uppercase tracking-wider" onClick={() => setFruitMenuOpen(false)}>
                  {link.name}
                ...
                <AnimatePresence>
                  {fruitMenuOpen && (
                    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} className="absolute top-full left-0 mt-3 w-72 rounded-3xl border border-border/60 bg-white shadow-2xl overflow-hidden">
                      {(fruitCategories.length ? fruitCategories : [
              ...
              <a key={link.name} href={link.href} className="text-foreground/80 hover:text-primary font-medium transition-colors relative group text-sm uppercase tracking-wider">
                {link.name}
