...
const WishlistSection = ({ items }: Props) => (
      ...
      {items.map((item) => (
        <motion.div key={item.id} whileHover={{ y: -6 }} className="group overflow-hidden rounded-[2rem] border border-border/60 bg-muted/20 shadow-sm">
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="flex items-center gap-2 text-sm text-foreground/70"><FiStar className="text-amber-500" /> {item.rating}</div>
            <p className="font-bold text-primary">{item.price.toLocaleString('vi-VN')}đ</p>
            <div className="flex gap-2">
              <button className="flex-1 inline-flex items-center justify-center gap-2 rounded-full bg-primary px-4 py-3 text-sm font-semibold text-white hover:bg-primary/90 transition-colors"><FiShoppingCart /> Add to cart</button>
              <button className="rounded-full border border-border px-4 py-3 text-rose-500 hover:bg-rose-50 transition-colors"><FiHeart /></button>
            </div>
          </div>
        </motion.div>
      ))}
